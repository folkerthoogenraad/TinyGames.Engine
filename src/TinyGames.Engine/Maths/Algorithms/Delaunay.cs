...
// https://github.com/nol1fe/delaunator-sharp/blob/master/DelaunatorSharp/Delaunator.cs
// TODO probably remove this stuff
// This is a good implementation, but I don't think this should be in the main engine stuff tbh
// Auxillary stuff.
// Credit to nol1fe for creating this port of the 'https://github.com/mapbox/delaunator'.
// In the future I might create TinyGames.Algorithms and implement all of these myself (of fun reasons). For now, MIT license stuff.
namespace TinyGames.Engine.Maths.Algorithms
{
    public class Delaunay
    {
        ...
        public DelaunayPoint[] Points { get; private set; }
        ...
        public Delaunay(DelaunayPoint[] points)
        {
            if (points.Length < 3)
            {
        ...
        private static DelaunayPoint Circumcenter(double ax, double ay, double bx, double by, double cx, double cy)
        {
            var dx = bx - ax;
            var dy = by - ay;
            var ex = cx - ax;
        ...
        public IEnumerable<DelaunayTriangle> GetTriangles()
        {
            ...
            {
                yield return new DelaunayTriangle(t, GetTrianglePoints(t));
            }
        }
        public IEnumerable<DelaunayEdge> GetEdges()
        {
            for (var e = 0; e < Triangles.Length; e++)
            {
                if (e > Halfedges[e])
                {
                    ...
                    yield return new DelaunayEdge(e, p, q);
                }
            }
        }
        public IEnumerable<DelaunayEdge> GetVoronoiEdges(Func<int, DelaunayPoint> triangleVerticeSelector = null)
        {
        ...
        public IEnumerable<DelaunayEdge> GetVoronoiEdgesBasedOnCircumCenter() => GetVoronoiEdges(GetTriangleCircumcenter);
        public IEnumerable<DelaunayEdge> GetVoronoiEdgesBasedOnCentroids() => GetVoronoiEdges(GetCentroid);

        public IEnumerable<VoronoiCell> GetVoronoiCells(Func<int, DelaunayPoint> triangleVerticeSelector = null)
        {
            if (triangleVerticeSelector == null) triangleVerticeSelector = x => GetCentroid(x);

            var seen = new HashSet<int>();
            var vertices = new List<DelaunayPoint>(10);    // Keep it outside the loop, reuse capacity, less resizes.
        ...
        public IEnumerable<DelaunayEdge> GetHullEdges() => CreateHull(GetHullPoints());

        public DelaunayPoint[] GetHullPoints() => Array.ConvertAll<int, DelaunayPoint>(hull, (x) => Points[x]);

        public DelaunayPoint[] GetTrianglePoints(int t)
        {
        ...
        public DelaunayPoint[] GetRellaxedPoints()
        {
            var points = new List<DelaunayPoint>();
        ...
        public IEnumerable<DelaunayEdge> GetEdgesOfTriangle(int t) => CreateHull(EdgesOfTriangle(t).Select(p => Points[p]));
        public static IEnumerable<DelaunayEdge> CreateHull(IEnumerable<DelaunayPoint> points) => points.Zip(points.Skip(1).Append(points.FirstOrDefault()), (a, b) => new DelaunayEdge(0, a, b)).OfType<DelaunayEdge>();
        public DelaunayPoint GetTriangleCircumcenter(int t)
        {
            var vertices = GetTrianglePoints(t);
            return GetCircumcenter(vertices[0], vertices[1], vertices[2]);
        }
        public DelaunayPoint GetCentroid(int t)
        {
            var vertices = GetTrianglePoints(t);
            return GetCentroid(vertices);
        }
        public static DelaunayPoint GetCircumcenter(DelaunayPoint a, DelaunayPoint b, DelaunayPoint c) => Circumcenter(a.X, a.Y, b.X, b.Y, c.X, c.Y);

        public static DelaunayPoint GetCentroid(DelaunayPoint[] points)
        {
            ...
            if (Math.Abs(accumulatedArea) < 1E-7f)
                return new DelaunayPoint();

            accumulatedArea *= 3f;
            return new DelaunayPoint(centerX / accumulatedArea, centerY / accumulatedArea);
        }

        #endregion GetMethods

        #region ForEachMethods
        public void ForEachTriangle(Action<DelaunayTriangle> callback)
        {
        ...
        public void ForEachTriangleEdge(Action<DelaunayEdge> callback)
        {
        ...
        public void ForEachVoronoiEdge(Action<DelaunayEdge> callback)
        {
        ...
        public void ForEachVoronoiCell(Action<VoronoiCell> callback, Func<int, DelaunayPoint> triangleVertexSelector = null)
        {
